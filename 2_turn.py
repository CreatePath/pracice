...
def draw_grid():
    for x in range(19):
        for y in range(19):
            ...
            grid_rect = pygame.Rect(0,0,20,20)
...
# timer class
class Timer:
    def __init__(self, min, sec, pos, color, clock_color):
        self.min = min
        self.sec = sec
        self.color = color
        ...
        while time_run:
                ...
                if event.type == pygame.MOUSEBUTTONDOWN:
                    ...
                    for grid_rect in grid_rects:
                        if grid_rect.collidepoint(event.pos):
                            self.placement(grid_rect, event.pos)
                            self.sec = curr_sec
                            time_run = False
                            break
                if time_run == False:
            ...
            for player in players:
                ...
                for position in player.stones:
                    pygame.draw.circle(screen, player.color, position, 10)
            pygame.display.update() 

# 흑/백 사용자 클래스
class Player(Timer):
    def __init__(self, min, sec, pos, num, color, clock_color):
        super().__init__(min, sec, pos, color, clock_color)
        self.stones = []
        self.num = num
...
# player class 호출
black = Player(10,0, (BOARD_SIZE,PLAYER_HEIGHT+40), 1, BLACK, WHITE)
white = Player(10,0, (BOARD_SIZE,INFO_HEIGHT+PLAYER_HEIGHT+40), 2, WHITE, BLACK)
players =[black, white]
